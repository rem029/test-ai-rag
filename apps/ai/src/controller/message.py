...
class MessageRequest(BaseModel):
    ...
    audioResponse: Optional[bool] = True
...
async def handle_message_logic(request: MessageRequest):
    ...
    if not request.session_id:
        ...
        logger.log_and_print(
            f"🆔 [cyan]Generated new session ID:[/cyan] [blue]{request.session_id}[/blue]"
        )
    ...
    if logger.session_logger is None:
        ...
        logger.log_and_print(
            f"🚀 [green]Session started:[/green] [blue]{request.session_id[:8]}...[/blue]"
        )
    ...
    if request.audioResponse and not request.text:
        logger.log_and_print(
            "⚠️ [yellow]Audio response requested without text input[/yellow]",
            log_level="warning",
        )
